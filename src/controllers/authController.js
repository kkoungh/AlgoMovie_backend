...
const register = async (req, res, next) => {
  try {
    ...
    if (!email || !password || !nickname) {
      return res
        .status(422)
        .json({ code: 'VALIDATION_ERROR', message: '이메일, 비밀번호, 닉네임은 필수입니다.' });
    }
...
const login = async (req, res, next) => {
  try {
    ...
    if (!email || !password) {
      return res
        .status(422)
        .json({ code: 'VALIDATION_ERROR', message: '이메일과 비밀번호를 입력해주세요.' });
    }
...
const refresh = async (req, res, next) => {
  try {
    ...
    if (!refreshToken) {
      return res
        .status(422)
        .json({ code: 'VALIDATION_ERROR', message: 'refreshToken이 필요합니다.' });
    }
